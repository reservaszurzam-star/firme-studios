...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({
    ...
    try {
      ...
      if (res?.error) {
        if (
          ...
          setErrorMsg(
            'El proveedor de Google aún requiere ser activado en el panel de Supabase. Puedes crear tu cuenta o ingresar con tu Correo y Contraseña en la pestaña "Crear cuenta".'
          );
        } else {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.user) {
        // Guardar credencial en el navegador también
        try {
      ...
      if (res.error) {
        if (res.error.toLowerCase().includes('already registered')) {
          setErrorMsg('Este correo ya está registrado. Por favor ingresa en la pestaña "Ingresar" con tu contraseña.');
        } else {
          setErrorMsg(res.error);
        }
        setLoadingAccountId(null);
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Intento de inicio de sesión real en Supabase Auth
      const res = await supabaseService.signInWithPassword(loginEmail, loginPassword);
      ...
      // 2. Si Supabase devuelve error de credenciales, verificar si existe en usuarios locales guardados
      ...
      if (foundUser) {
        if (foundUser.password && foundUser.password !== loginPassword) {
          setErrorMsg('Contraseña incorrecta. Por favor verifica tu clave secreta.');
          setLoadingAccountId(null);
          return;
        }
      ...
      // Si no coincide
      setErrorMsg('Contraseña incorrecta o correo no registrado. Por favor verifica tus credenciales o regístrate.');
      setLoadingAccountId(null);
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto pr-1 flex-1">
          {/* -------------------------------------------------------------
              TAB 1: CREAR CUENTA NUEVA CON CORREO & CONTRASEÑA REAL
              ------------------------------------------------------------- */}
          {authMode === 'register' && (
            <div className="space-y-3.5">
              {/* Atajo rápido con Google */}
              <button
                type="button"
                onClick={handleGoogleSignIn}
                disabled={loadingAccountId !== null}
                className="w-full bg-white hover:bg-[#F7F5F0] border border-[#DDD5C9] hover:border-[#B5654A] py-2.5 px-4 rounded-xl font-medium text-xs sm:text-sm text-[#1A1815] shadow-2xs flex items-center justify-center gap-2.5 transition-all cursor-pointer disabled:opacity-50"
              >
                ...
                <span>{loadingAccountId === 'google' ? 'Conectando con Google...' : 'Registrarme con Google en 1 Clic'}</span>
              </button>
              ...
              <div className="relative my-2">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-wider text-[#6B655C]">
                  <span className="bg-[#FAF8F5] px-2 font-medium">o con correo y contraseña</span>
                </div>
              </div>
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-3">
                ...
                <div className="p-2.5 rounded-xl bg-[#F1ECE5] text-[11px] text-[#6B655C] flex items-center gap-2 border border-[#E4DED4]">
                  <Users className="w-4 h-4 text-[#B5654A] shrink-0" />
                  <span>
                    Tu cuenta guarda tu historial de clases, créditos y tu cama Reformer asignada.
                  </span>
                </div>
          ...
          {authMode === 'login' && (
            <div className="space-y-3.5">
              ...
              >
                ...
                <span>{loadingAccountId === 'google' ? 'Conectando con Google...' : 'Iniciar sesión con Google en 1 Clic'}</span>
              </button>
          ...
          {/* -------------------------------------------------------------
              TAB 3: GOOGLE OAUTH DIRECTO (1 CLIC REAL)
              ------------------------------------------------------------- */}
          {authMode === 'google' && (
            <div className="space-y-4 py-2">
              <div className="bg-white border border-[#DDD5C9] p-5 rounded-2xl text-center space-y-3 shadow-xs">
                <div className="w-12 h-12 rounded-full bg-white border border-gray-200 shadow-xs flex items-center justify-center mx-auto">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#6B655C] max-w-xs mx-auto mt-1 leading-relaxed">
                    Conecta directamente con tu cuenta personal de Google en 1 solo clic. Sin contraseñas que recordar y sincronizado con Supabase.
                  </p>
                </div>
                ...
                <button
                  ...
                  onClick={handleGoogleSignIn}
                  disabled={loadingAccountId !== null}
                  className="w-full bg-white hover:bg-[#F7F5F0] border border-[#DDD5C9] hover:border-[#B5654A] py-3 px-4 rounded-xl font-semibold text-xs sm:text-sm text-[#1A1815] shadow-xs flex items-center justify-center gap-2.5 transition-all cursor-pointer disabled:opacity-50"
                >
                  <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24">
                    <path
                      fill="#4285F4"
                  ...
                  <span>{loadingAccountId === 'google' ? 'Conectando con Google...' : 'Continuar con mi Cuenta de Google'}</span>
                </button>
              </div>
              ...
              <div className="text-center">
                <button
                  ...
                  className="text-xs text-[#6B655C] hover:text-[#B5654A] transition-colors cursor-pointer underline"
                >
                  O prefiere registrarse con correo y contraseña
                </button>
              </div>
            </div>
          )}
